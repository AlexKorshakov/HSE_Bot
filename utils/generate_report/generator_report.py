...
async def create_report_from_other_method(message: types.Message, dataframe=None, images_file_list=None):
    ...
    if is_created is None:
        ...
        await bot.send_message(message.from_user.id, MESSAGES["workbook not create"])
        return

    workbook = await get_workbook(fill_report_path)
    if workbook is None:
        logger.warning('error! Workbook not found!')
        await bot.send_message(message.from_user.id, MESSAGES["workbook not found"])
        return

    worksheet = await get_worksheet(workbook, index=0)
    if worksheet is None:
        logger.warning('error! worksheet not found!')
        await bot.send_message(message.from_user.id, MESSAGES["worksheet not found"])
        return
...
async def create_report(message: types.Message):
    ...
    if fill_report_path is None:
        ...
        await bot.send_message(message.from_user.id, MESSAGES["fill_report_path not found"])
        return
    ...
    if dataframe is None:
        ...
        await bot.send_message(message.from_user.id, MESSAGES["dataframe not found"])
        return
