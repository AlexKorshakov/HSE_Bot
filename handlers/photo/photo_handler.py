...
from utils.secondary_functions.get_filepath import get_filepath
...
async def photo_handler(message: types.Message):
    ...
    report_data["now"] = datetime.datetime.now()
    report_data["filepath"] = await get_filepath(report_name_mod)
    ...
    await message.photo[-1].download(destination=await get_filepath(report_name_mod))
