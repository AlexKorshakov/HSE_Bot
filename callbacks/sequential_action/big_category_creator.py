...
async def big_category(call: types.CallbackQuery, big_menu_list, num_col=2):
    ...
    while i_start < num:
        i_stop = i_start + 10
        try:
            await select_category(call.message, some_list=big_menu_list[i_start:i_stop], num_col=num_col)

        except Exception as callback_err:
            logger.error(f"{repr(callback_err)}")
            await select_category(call.message, some_list=big_menu_list[i_start:num], num_col=num_col)
